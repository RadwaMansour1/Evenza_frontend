...
export class OrganizerWalletComponent implements OnInit, OnDestroy {
    ...
    handleSubmitPaymentInfo(): void {
        ...
        const token = sessionStorage.getItem(CONSTANTS.token) || localStorage.getItem(CONSTANTS.token);
        let organizerId = '';
        ...
        this.organizerService.updateAndSaveTransferInfo(transferInfo)
    ...
    loadPaymentInformation(): void {
      this.loading = true;
      this.organizerService.getTransferInfo()
          .subscribe({
            next: (response: TransferInfoResponse) => {
              const transferInfo = response.data;

                  if (transferInfo) {
                      this.paymentMethod = transferInfo.type as 'E-Wallet' | 'InstaPay' | 'Bank' || 'Bank';
